...
interface TimerStore {
  ...
  pause: () => void;
  resume: () => void;
  stop: () => { categoryId: string; todoId: string | null; durationMinutes: number; startTime: string; endTime: string } | null;
  reset: () => void;
  getElapsedMs: () => number;
  addTestHour: () => void;
}
...
export const useTimerStore = create<TimerStore>()(
  persist(
    (set, get) => ({
      ...
      stop: () => {
        const state = get();
        if (state.status === 'idle' || !state.categoryId || !state.startedAt) return null;
      ...
      getElapsedMs: () => {
        ...
        if (status === 'running' && lastResumedAt) {
          const currentSegmentMs = Date.now() - new Date(lastResumedAt).getTime();
