...
function GridContent() {
  const { chapters, addChapter, updateChapter, deleteChapter, hoursPerDay } = useChapterStore();
  ...
  // View range
  const [viewStartOverride, setViewStartOverride] = useState<string | null>(null);
  const [viewEndOverride, setViewEndOverride] = useState<string | null>(null);

  // Saved range (restored when closing a chapter)
  const savedRange = useRef<{ start: string | null; end: string | null }>({ start: null, end: null });
  ...
  const viewStart = viewStartOverride ?? globalRange?.start ?? '';
  const viewEnd = viewEndOverride ?? globalRange?.end ?? '';
  ...
  const chapterHours = useMemo(() => {
    ...
    for (const ch of chapters) {
      ...
      const totalMins = chapterEntries.reduce((sum, e) => sum + e.durationMinutes, 0);
      map.set(ch.id, totalMins / 60);
    }
    return map;
  }, [chapters, entries]);

  const chapterCategoryBreakdown = useCallback((chapterId: string) => {
    ...
    const totalMins = chapterEntries.reduce((s, e) => s + e.durationMinutes, 0);
    const map = new Map<string, number>();
    ...
    return Array.from(map.entries())
      .map(([catId, mins]) => {
        ...
        return { category: cat, minutes: mins, percent: totalMins > 0 ? (mins / totalMins) * 100 : 0 };
      })
      .filter((b) => b.category)
  ...
  const handleChapterClick = (chapterId: string) => {
    if (expandedChapterId === chapterId) {
      ...
      setViewStartOverride(savedRange.current.start);
      setViewEndOverride(savedRange.current.end);
    } else {
      // Opening: save current range, zoom to chapter
      savedRange.current = { start: viewStartOverride, end: viewEndOverride };
      const ch = chapters.find((c) => c.id === chapterId);
      if (ch) {
        setViewStartOverride(ch.startDate);
        setViewEndOverride(ch.endDate);
      }
  ...
  const handleResetView = () => {
    setViewStartOverride(null);
    setViewEndOverride(null);
    setExpandedChapterId(null);
  };
  ...
  const saveEditChapter = () => {
    if (!editingChapterId || !editName.trim() || !editStartDate || !editEndDate) return;
    if (editEndDate < editStartDate) return;
    const nextTasks = editTasks.filter((t) => t.trim().length > 0);
    const prevTasks = chapters.find((c) => c.id === editingChapterId)?.tasks ?? [];
  ...
  const isCustomView = viewStartOverride !== null || viewEndOverride !== null;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* New Chapter Form */}
      {showNewChapter && (
        <div className="rounded-lg border border-card-border bg-card p-4 space-y-3" onKeyDown={handleChapterKeyDown}>
          ...
          <div className="flex flex-col gap-3">
            ...
            <div>
              <label className="mb-1 block text-xs text-muted">Chapter Tasks</label>
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                  {newTasks.map((task, idx) => (
                    ...
                    >
                      {task}
                      <button
                        onClick={() => setNewTasks((prev) => prev.filter((_, i) => i !== idx))}
            ...
            {newStartDate && newEndDate && newEndDate >= newStartDate && (
              <div className="text-xs text-muted">
                {daysBetween(newStartDate, newEndDate)} days &middot; {formatDate(newStartDate)} &ndash; {formatDate(newEndDate)}
              </div>
            )}
      ...
      {dates.length > 0 && (
        <div className="flex flex-wrap items-end gap-3">
          <div>
            <label className="mb-1 block text-[10px] text-muted uppercase tracking-wider">From</label>
            <input
              type="date"
              value={viewStart}
              onChange={(e) => { setViewStartOverride(e.target.value); setExpandedChapterId(null); }}
          ...
          <div>
            <label className="mb-1 block text-[10px] text-muted uppercase tracking-wider">To</label>
            <input
              type="date"
              value={viewEnd}
              onChange={(e) => { setViewEndOverride(e.target.value); setExpandedChapterId(null); }}
      ...
      {dates.length > 0 ? (
        <div className="overflow-x-auto -mx-4 px-4">
          <div style={{ minWidth: `${Math.max(dates.length * (dates.length > 30 ? 14 : dates.length > 15 ? 22 : 32), 280)}px` }}>
            <HistogramGrid
              dates={dates}
        ...
        <div className="flex flex-col items-center gap-4 rounded-lg border border-card-border bg-card px-8 py-16 text-center">
          ...
          <p className="text-sm text-muted max-w-sm">
            Create a new chapter to start tracking your productivity. Pick a name
            and a date range, then watch your progress fill in.
          </p>
          <button
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-2">
            {[...chapters].reverse().map((ch) => {
              ...
              const breakdown = isExpanded ? chapterCategoryBreakdown(ch.id) : [];
              const maxMins = breakdown.length > 0 ? Math.max(...breakdown.map(b => b.minutes)) : 0;
              return (
                <div key={ch.id}>
                  <div
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-muted">
                        {days} days &middot; Started {formatDate(ch.startDate)} &middot; {Math.round(hours)}hrs
                      </div>
                    </div>
                    <svg
                  ...
                  {isExpanded && (
                    <div className="ml-6 mt-2 mb-1 space-y-1.5 border-l border-card-border pl-3">
                        ...
                        <div className="space-y-3">
                              ...
                              <input
                                ...
                                onKeyDown={(e) => {
                                  if (e.key === 'Enter' && editTaskInput.trim()) {
                                    e.preventDefault();
                                    setEditTasks((prev) => [...prev, editTaskInput.trim()]);
                                    setEditTaskInput('');
                                  }
                                }}
                              />
                          ...
                          <div className="flex items-center gap-2 justify-end">
                            ...
                            <button
                              ...
                              disabled={!editName.trim() || !editStartDate || !editEndDate || editEndDate < editStartDate}
                        ...
                        <>
                          ...
                          {ch.tasks && ch.tasks.length > 0 && (
                            <div className="space-y-1">
                              <div className="text-[10px] uppercase tracking-wider text-muted">Tasks</div>
                              <div className="space-y-1">
                                {ch.tasks.map((task, idx) => (
                                  <div key={`${task}-${idx}`} className="text-xs text-foreground/70">
                                    • {task}
                                  </div>
                                ))}
                              </div>
                      ...
                      {breakdown.length === 0 ? (
                        <p className="text-xs text-muted">No entries in this chapter yet.</p>
                      ) : (
